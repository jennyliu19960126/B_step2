...
if "GP_OUTPUT_ROOT" not in os.environ:
    os.environ["GP_OUTPUT_ROOT"] = str(Path(__file__).resolve().parent / "runs" / ("output_token_pilot_" + time.strftime("%Y%m%d_%H%M%S")))
...
TEST_PARAMETERS = {
    ...
    "hall_of_fame": 20,
    "n_components": 10,
    # Warm start evaluates 5x population, then one evolutionary generation.
    "generations": 1,
    "tournament_size": 10,
    "init_depth": [1, 4],
    "n_jobs": 4,
...
def main() -> int:
    ...
    raw_result = output_dir / "raw_result_token_pilot.csv"
    original_result = output_dir / "original_result_token_pilot.csv"
