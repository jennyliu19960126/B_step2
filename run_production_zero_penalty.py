...
The production configuration remains untouched.  Run this script with:
    ...
    python run_production_zero_penalty.py
"""
...
if "GP_OUTPUT_ROOT" not in os.environ:
    os.environ["GP_OUTPUT_ROOT"] = str(Path(__file__).resolve().parent / "runs" / ("production_249_penalty0_" + time.strftime("%Y%m%d_%H%M%S")))
...
TEST_PARAMETERS = {
    ...
    "llm_interpretability_weight": 0.0,
}
...
def main() -> int:
    ...
    manifest["fitness_formula"] = "abs(mean(signed quarterly IC)); zero penalty, no LLM requests"
    _write_json(manifest_path, manifest)
