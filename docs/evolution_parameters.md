...
This command stays in the foreground; use an existing persistent terminal session for a long run. Each job writes `launcher.log`; the batch writes `plan.json` and `batch_status.json`. Failures are recorded and other combinations continue. Existing batch directories are refused, preventing accidental overwrite. Each group automatically runs GP, materializes the union of the 4/8-generation catalogs once, and filters each snapshot separately. The shared filter uses unpenalized `base_fitness >= 0.04` and rejects correlations above 0.6. Training `min_raw_ic` remains 0/0.01. Postprocessing uses at most the same 40 workers, sequentially after training, so the three groups remain bounded at 120 workers total. Final lists are `backtest/production/postprocess/through_gen4/selected.csv` and `through_gen8/selected.csv`. The postprocess summary and batch status record before/after counts. Empty snapshots produce zero-count results; postprocessing failures mark the group failed.
